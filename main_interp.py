...
if __name__ == "__main__":
    ...
    collections = {} 
    nc = 0 
    for (x,y) in mwi_datas.keys():
        col = {}
        col['map'] = str(mwi_datas[(x,y)]['file'])
        col['x'] = x
        col['y'] = y
        col['z'] = 0
        col['fingerprints'] = []
        #print("mwi_wifi", x, y, len(mwi_datas[(x,y)]['wifi']))
        #continue 
        if len(mwi_datas[(x,y)]['wifi']) > 0:
            fp = {}
            fp['timestamp'] = 0
            fp['wifi'] = {}
            fp['ble'] = {}
            
            for wifi in mwi_datas[(x,y)]['wifi']:
                #print("   wifi ", wifi[2], wifi[3])
                fp['wifi'][wifi[2]] = {}
                fp['wifi'][wifi[2]]['rssi'] = [int(wifi[3])]
                if fp['timestamp'] == 0:
                    fp['timestamp'] = int(wifi[0])
                #else:
                #    assert(fp['timestamp'] == int(wifi[0]))
            for ble in mwi_datas[(x,y)]['ibeacon']:
                #print("   ble ", ble[1], ble[2])
                fp['ble'][ble[1]] = {}
                fp['ble'][ble[1]]['rssi'] =  [int(ble[2])]
                
        col['fingerprints'] = [] 
        col['fingerprints'].append(fp)
        collections[f"collection{nc}"] = col
        nc = nc + 1
                
    with open(os.path.basename(floor_data_dir)+'.int.json', "w+") as outfile:
