    ...
    for path_filename in path_filenames:
        ...
        for p in wifi_datas:
            while tp + 1 < len(posi_datas) and float(p[0]) > float(posi_datas[tp+1][0]):
                tp = tp + 1
            # print(p[0], p[2], p[3], tp, posi_datas[tp][0], f"col{nc - len(posi_datas) + tp}")
            wf = collections[f"collection{nc - len(posi_datas) + tp}"]["fingerprints"][0]["wifi"]
            if p[2] in wf:
                ...
                wf[p[2]]['rssi'] = [int(p[3])]
        tp = 0     
        for p in ibeacon_datas:
            while tp + 1 < len(posi_datas) and float(p[0]) > float(posi_datas[tp+1][0]):
                tp = tp + 1
            # print(p[0], p[1], p[2], tp, posi_datas[tp][0])
            ble = collections[f"collection{nc - len(posi_datas) + tp}"]["fingerprints"][0]["ble"]
            if p[1] in ble:
